...
function index(req, res) {
  ...
  function show(req, res) {
    ...
    Flight.findById (req.params.id,function(err, flight) {
      let addedDests = flight.destinations.map(dest => dest.airport);
      let allowedDests = ['AUS', 'DFW', 'DEN', 'LAX', 'SAN'].filter(dest => !addedDests.includes(dest));      
      Ticket.find({flight: flight._id}, function(err, tickets){
      res.render('flights/show', { title: 'Flight Detail', section: 'Flight Details',  depDateShow, flight, tickets, allowedDests});
    });
  });
  }
